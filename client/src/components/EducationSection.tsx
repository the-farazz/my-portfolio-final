    ...
    <section className="py-20 px-4 lg:px-8 bg-[rgb(40,40,40)] light:bg-gray-50">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="space-y-8">
            {education.map((edu, index) => (
              <div key={index} className="relative flex items-start">
                ...
                <div className="md:ml-20 bg-[rgb(30,30,30)] p-8 rounded-xl w-full hover:shadow-xl transition-all duration-300 hover:transform hover:scale-[1.02] light:bg-white light:border light:border-gray-200">
                  <div className="flex items-start space-x-6">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-2">
                        ...
                        <span className="text-sm text-gray-400 font-medium mt-1 md:mt-0 light:text-gray-500">
                          {edu.period}
                        </span>
                      </div>

                      <h4 className="text-lg text-white font-medium light:text-gray-900">
                        {edu.institution}
                      </h4>
                      <p className="text-sm text-gray-500 mb-3 light:text-gray-500">{edu.location}</p>
        ...
        <div className="mt-16 bg-[rgb(30,30,30)] p-8 rounded-xl light:bg-white light:border light:border-gray-200">
          <h3 className="text-xl font-semibold text-[yellow] mb-6 text-center">
            Academic Foundation
          </h3>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { icon: '💻', title: 'Software Development', desc: 'Strong foundation in programming principles and software engineering' },
              { icon: '🧠', title: 'Problem Solving', desc: 'Analytical thinking and systematic approach to technical challenges' },
              { icon: '📈', title: 'Academic Excellence', desc: 'Consistent academic performance and dedication to learning' },
            ].map((item, i) => (
              <div key={i} className="text-center">
                <div className="w-12 h-12 bg-[yellow] rounded-full flex items-center justify-center mx-auto mb-3">
                  <span className="text-[rgb(30,30,30)] text-xl">{item.icon}</span>
                </div>
                <h4 className="font-semibold text-white mb-2 light:text-gray-900">{item.title}</h4>
                <p className="text-gray-400 text-sm light:text-gray-600">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
